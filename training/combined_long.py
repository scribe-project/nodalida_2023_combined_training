...
wandb.init(project="combined_full_nb_model_voxrex")

dataset_id = "scribe-project/NPSCRundkast_nb_total"

print("Load datasets")
cache_dir = data_dir / "huggingface_cache/"
dataset_train = load_dataset(
    dataset_id, split="train", use_auth_token=True, cache_dir=cache_dir,
)
dataset_valid = load_dataset(
    dataset_id, split="validation", use_auth_token=True, cache_dir=cache_dir,
)
dataset_test = load_dataset(
    dataset_id, split="test", use_auth_token=True, cache_dir=cache_dir,
)
...
Path(repo_name).mkdir(parents=True, exist_ok=True)
print("Push to hub")
tokenizer.push_to_hub(repo_id=repo_id, use_temp_dir=False, repo_path_or_name=repo_id)
...
processor.push_to_hub(repo_id=repo_id, use_temp_dir=False, repo_path_or_name=repo_id)
...
def prepare_dataset(batch):
    ...
    batch["input_values"] = processor(audio["array"], sampling_rate=16000).input_values[
        0
    ]
